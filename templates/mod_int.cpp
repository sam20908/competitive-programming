struct mint {
  ...
  friend mint operator-(const mint &a, const mint &b) { return {(a.v - b.v + M) % M}; }
  friend mint operator-(long long a, const mint &b) { return {(a - b.v + M) % M}; }
  friend mint operator-(const mint &a, long long b) { return {(a.v - b + M) % M}; }
  friend mint operator*(const mint &a, const mint &b) { return {(a.v * b.v) % M}; }
  friend mint operator*(long long a, const mint &b) { return {(a * b.v) % M}; }
  friend mint operator*(const mint &a, long long b) { return {(a.v * b) % M}; }
